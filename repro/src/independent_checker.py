...
def _claim_3(main: dict) -> dict:
    norms = {}
    max_error = 0.0
    max_reported_quadrature_error = 0.0
    for name, variance_sum in {
        ...
        norms[name] = value
        max_error = max(max_error, abs(value - main["example_2"]["norms"][name]))
        max_reported_quadrature_error = max(max_reported_quadrature_error, error)
    # Independently validate the universal 1/65 calculus step.
    lower_bound = float(norm.cdf(0.75) - norm.cdf(0.70))
    proof_step_holds = lower_bound >= 1.0 / 65.0
    example_holds = (
        main["example_2"]["minimum_asymptotic_power_advantage"] >= 1.0 / 65.0
    )
    return {
        "passed": bool(max_error < 1e-10 and proof_step_holds and example_holds),
        "method": "adaptive quadrature of Gaussian kernel expectations plus independent normal-CDF bound",
        "quadrature_norms": norms,
        "maximum_error": max_error,
        "maximum_reported_quadrature_error": max_reported_quadrature_error,
        "cdf_interval_0.70_to_0.75": lower_bound,
        "target_1_over_65": 1.0 / 65.0,
        "published_proof_step_holds": bool(proof_step_holds),
        "paper_gaussian_example_holds": bool(example_holds),
    }
